...
let c = fs.readFileSync("src/systems/combat.ts", "utf8");
c = c.replace(
  `    if (unit.freezeT > 0) {
...
fs.writeFileSync("src/systems/combat.ts", c, "utf8");
console.log("combat freeze tick deduped");
...
import { towerRange } from "../src/systems/towers.ts";
...
console.log("iniesta freeze ok", { freezeT: foe.freezeT.toFixed(2), foeHp: foe.hp });

console.log("SYMMETRY + FREEZE SMOKE OK");
`;

fs.writeFileSync("scripts/smoke-freeze.mts", smoke, "utf8");
console.log("wrote smoke-freeze.mts");
