...
let m = fs.readFileSync("src/config/match.ts", "utf8");
m = m.replace("princessMaxHp: 5,", "princessMaxHp: 20,");
m = m.replace("kingMaxHp: 5,", "kingMaxHp: 20,");
if (!m.includes("princessMaxHp: 20")) {
  console.error("HP patch failed", m.match(/.*MaxHp.*/g));
  process.exit(1);
}
fs.writeFileSync("src/config/match.ts", m, "utf8");
console.log("tower HP -> 20");
...
function attackRange(state: GameState, unit: Unit): number {
  ...
  if (def.attackMode === "melee") {
    return (def.meleeRange ?? 36) * state.config.rangeScale;
  }
  return unit.range;
}
...
export function updateTargeting(state: GameState): void {
  ...
  for (const unit of state.units) {
    ...
    if (def.attackMode === "hybrid") {
      const meleeRange = (def.meleeRange ?? 36) * state.config.rangeScale;
