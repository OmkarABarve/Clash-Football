/**
 * Pen → Dive with verified symbol names from this codebase.
 */
const fs = require("fs");
...
// types
{
  ...
  if (s.includes("towerHpDelta?") && !s.includes("yellowAttackSpeedMult")) {
    s = s.replace(
      "  towerHpDelta?: number;\n  enabled?: boolean;",
      "  towerHpDelta?: number;\n  /** Dive: AS multiplier on yellow (0.8 = −20%). */\n  yellowAttackSpeedMult?: number;\n  enabled?: boolean;",
    );
  }
  // also handle comment variants
  s = s.replace(
    /\/\*\* Pen:[^*]+\*\/\n  towerHpDelta\?: number;/,
    "/** Dive: HP removed when cast on a tower. */\n  towerHpDelta?: number;",
  );
  if (!s.includes("yellowCard:")) {
    if (!s.includes("freezeT: number;")) throw new Error("freezeT field missing in Unit");
    s = s.replace(
      "  freezeT: number;\n}",
...
import { enemyGoals } from "./goals";`,
  ...
  if (idx < 0) throw new Error("castPen not found");
  let cut = idx;
