/**
 * Apply Dive rename using exact identifiers discovered at runtime.
 */
const fs = require("fs");
...
const GEO = {
  ...
  closestUnit: "closestPointOnUnit",
  closestGoal: "closestPointOnGoal",
  center: "goalCenter",
  pointIn: "pointInGoal",
};
const GOALS = { enemy: "enemyGoals" };
const DMG = { goal: "applyGoalDamage" };
const FAC = { proj: "createProjectile" };
const CARDS = { spell: "getSpellDef" };
const SP = { enemy: "enemyOf" };

// types
{
  let s = read("src/config/types.ts");
  s = s.replace(/"highline" \| "pen"/, '"highline" | "dive"');
  if (!s.includes("yellowAttackSpeedMult")) {
    s = s.replace(
      /towerHpDelta\?: number;/,
...
// combat
{
  ...
  if (s.includes("unit.cooldownLeft = def.attackCooldown;") && !s.includes("yellowCard ?")) {
    s = s.replace(
...
// spells — rebuild cast section
{
  let s = read("src/systems/spells.ts");
  // replace geometry/goals imports
  if (s.includes('from "./geometry"')) {
    s = s.replace(
  ...
  if (s.includes('from "./goals"')) {
    s = s.replace(
      /import \{[^}]+\} from "\.\/goals";/,
      `import { ${GOALS.enemy} } from "./goals";`,
    );
  }

  const idx = s.indexOf("export function castPen(");
  if (idx < 0) throw new Error("castPen not found — already patched?");
  let cut = idx;
  const doc = s.lastIndexOf("/**", idx);
  if (doc >= 0 && idx - doc < 400) cut = doc;
...
export function findDiveTarget(
  ...
  const foe = ${SP.enemy}(caster);
  const hitR = DIVE_HIT_TILES * state.config.tileSize;

  let bestGoal: { id: string; d: number } | null = null;
  for (const goal of ${GOALS.enemy}(state, foe)) {
    if (goal.hp <= 0) continue;
    let d = 0;
    if (!${GEO.pointIn}(x, y, goal)) {
      const p = ${GEO.closestGoal}(goal, x, y);
      d = ${GEO.dist}(x, y, p.x, p.y);
    }
  ...
  for (const unit of state.units) {
    ...
    const p = ${GEO.closestUnit}(unit, x, y, state.config.tileSize);
...
export function castDive(
  ...
  const f = ${GEO.footprint}(unit, state.config.tileSize);
...
// intents
{
  ...
  s = s.replace(
    /\} else if \(def\.spell === "pen"\) \{\s*castPen\(state, intent\.side, intent\.x, intent\.y\);\s*\}/,
...
// hud
{
  ...
  s = s.replace(/King −1/g, "Tower −1 / YC");
  s = s.replace(/King -1/g, "Tower −1 / YC");
...
// ai
{
  ...
  if (s.includes('"pen"')) {
    write("src/systems/ai.ts", s.replace(/"pen"/g, '"dive"'));
  }
}
...
// render
{
  ...
  if (!s.includes('def.spell === "dive"')) {
    s = s.replace(
      /if \(isSpellDef\(def\) && def\.spell === "highline"\) \{[\s\S]*?return;\n  \}\n  if \(isSpellDef\(def\)\) return;/,
